...
/* Prend un descripteur de fichier en entrée, va lire la map dans le fichier et la renverra dans un tableau a 2 dimensions */
level *read_level(int fd){
  int i = 0,j = 0,count = 1;
  level *m = malloc(sizeof(level));
  char buffer[BUFMAX];
  read(fd,buffer,BUFMAX); // On lit tout le fichier comme une grande chaine de caractères
  char *s=strtok(buffer," ");// On la découpe ensuite espace par espace ou \n par \n
  ...
  for(i=m->hauteur-1; i>=0; i--) m->map[i] = calloc(m->largeur,sizeof(point));
	/*Chargement de la map */
  for(i=m->hauteur-1; i>=0; i--){
	/*Bas de la map charge a partir du fichier */
    if(i>=HAUTEUR-haut_debut)	s=strtok(NULL,"\n"); //lit toute la ligne de le map
    for(j=0;j< m->largeur;j++){
      if(i<HAUTEUR-haut_debut ) m->map[i][j].val = '0';
	else{
	  m->map[i][j].val=s[j];
	  if(s[j]=='1') m->map[i][j].color=0; // 0 est la couleur de base, aucune brique n'aura cette couleur (pour distinguer)
	}
     }
  }	
...
void aff_map(level *m){
  ...
  for(j=0;j< m->largeur;j++){
	...
	case 0: write(1,"\033[40m\u2588\u2588\033[0m",15); break;
...
void add_brique(level* m, brique* br){ // ajoute la brique a la map, conserve la couleur
  int i, j;
